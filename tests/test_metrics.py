...
def test_render_shape_and_math():
    snap = {"total": 5, "modules": [
        {"module": "nsfw", "runs": 3, "detections": 2},
        {"module": "pii", "runs": 2, "detections": 0},
    ]}
    text = render_prometheus(snap, window_seconds=86400)
    m = _parse(text)
...
def test_render_has_help_and_type_headers():
    ...
    text = render_prometheus({"total": 0, "modules": []}, 60)
    for name in ("guardrails_requests_total", "guardrails_module_requests",
                 "guardrails_module_detections", "guardrails_module_no_detections"):
...
# --- HTTP-эндпоинт -----------------------------------------------------------
def _key(client):
    return client.post("/admin/api-keys", json={"name": "m"}, headers=ADMIN).json()["key"]


def _add_nsfw_word(client, word):
    r = client.post("/admin/nsfw", json={"name": "t", "text": word}, headers=ADMIN)
    assert r.status_code == 200, r.text


def test_metrics_reflect_real_runs():
    client, _ = make_client()
    client.app.state.metrics_cache = None  # свежий старт
    key = _key(client)
    _add_nsfw_word(client, "плохое")
    hdr = {"X-API-Key": key}

    # 2 прогона nsfw: один со сработкой (запрещённое слово), один без
    assert client.post("/v1/detect/nsfw", json={"text": "плохое"}, headers=hdr).json()["NSFW_DETECT"] is True
    assert client.post("/v1/detect/nsfw", json={"text": "привет"}, headers=hdr).json()["NSFW_DETECT"] is False
    # 1 прогон relevant
    client.post("/v1/detect/relevant", json={"text": "привет"}, headers=hdr)
...
def test_metrics_are_cached_until_invalidated():
    client, _ = make_client()
    ...
    client.post("/v1/detect/nsfw", json={"text": "привет"}, headers=hdr)
