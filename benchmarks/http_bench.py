...
def test_single_latency(key, n=500):
    ...
    for _ in range(n):
        t = time.perf_counter()
        requests.post(f"{BASE}/detect/pii", headers=h, json={"text": TEXT}, timeout=5)
...
def test_batch(key, batch_size=500, requests_n=40):
    ...
    # прогрев
    requests.post(f"{BASE}/detect/pii/batch", headers=h, json={"texts": texts}, timeout=30)
    start = time.perf_counter()
    for _ in range(requests_n):
        r = requests.post(f"{BASE}/detect/pii/batch", headers=h, json={"texts": texts}, timeout=30)
        r.raise_for_status()
...
def _worker(args):
    ...
    while time.perf_counter() < end:
        s.post(f"{BASE}/detect/pii", headers=h, json={"text": TEXT}, timeout=5)
        count += 1
    return count
